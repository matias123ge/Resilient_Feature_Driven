"""
DA trading with missing data/dual price market/ fixed prices
...
def resilient_crossval_actual_costs(x, y, p_up, p_down, group_col, l1_weights, num_runs = 10): 
    ...
    X_train, X_valid, y_train, y_valid = sklearn.model_selection.train_test_split(x, y, test_size = 0.4, random_state = 42, shuffle = False)
    p_up_train, p_up_valid, p_down_train, p_down_valid = sklearn.model_selection.train_test_split(p_up, p_down, test_size = 0.4, random_state = 42, shuffle = False)
...
def params():
    ...
    params['store_folder'] = 'DA-CVAR-case' # folder to save stuff (do not change)
    ...
    params['CVaRWeight'] = 0.5 
    params['CVaRConf'] = 0.95
...
num_feat_per_grid = config['Nr_Feat']
K_parameter = np.arange(config['K value']+1)
...
if config['train'] == True: 
    ...
    for K in K_parameter:
        ...
        fdr.fit(xtrain_sc, ytrain, group_col, fix_col, verbose=-1, 
...
#%%
# Feature driven model CVaR 
output_file_name = f'{cd}\\{case_folder}\\trained-models\\CVAR-model.pickle'
if config['train'] == False: 
    cvar = CVaR_reg( pen_up = pen_up, pen_down = pen_down, k = config['CVaRWeight'],alpha = config['CVaRConf'])
    cvar.fit(xtrain_sc, ytrain)
    print("Feat-driven coef")
    if config['save']:
        with open(output_file_name, 'wb') as handle:
            pickle.dump(cvar, handle)
else:
    with open(output_file_name, 'rb') as handle:    
            cvar = pickle.load(handle)
...
retrain_models, retrain_pred, retrain_comb = retrain_model(xtrain_sc, ytrain.values, xtest_sc,
                                                           pen_up, pen_down, group_col, fix_col, max(K_parameter), base_loss='l2')

#%% L1 regularized feature-driven models

# Feature-driven w l1 regularization w/ standard crossvalidation (e.g., lasso)
...
list_alpha = np.array([0.001, 0.01, 0.05, 0.1, 0.5, 1, 5, 10])

if config['trainReg']:
    
    if config['fixed_costs']:
        best_aplha, val_errors, _ = resilient_crossval_fixed_costs(xtrain.values, ytrain.values, pen_up, pen_down, group_col, list_alpha, 10_000)
    else:
        best_aplha, val_errors, _ = resilient_crossval_actual_costs(xtrain.values, ytrain.values, pen_up_train.values, pen_down_train.values, group_col, list_alpha, 10_000)
    ...
    if config['save']:
        with open(cd + '\\trained-models\\' +'det-feature-expanded-reg-group.pickle', 'wb') as handle:
            pickle.dump(mlr_l1, handle)
            
else:
    with open(cd + '\\trained-models\\' +'det-feature-expanded-reg-group.pickle', 'rb') as handle:    
...
models = ['SAA', 'Feature Driven', 'Feature Driven-L1','BASE-retrain','CVAR'] + ['FDRR_reform'+str(k) for k in K_parameter] + ['FDRR_affine'+str(k) for k in K_parameter]
if config['impute']:
    labels = ['SAA$^{imp}', 'Feature Driven$^{imp}$', 'Feature Driven-L1','Base Retrain','CVAR'] + ['FDRRref$^'+str(k)+'$' for k in K_parameter] +['FDRRaff$^'+str(k)+'$' for k in K_parameter]
    #labels = ['BASE$^{imp}$', 'BASE$_{\ell_2}^{imp}$', 'BASE$_{\ell_1}^{imp}$', 'RF$^{imp}$', 'RETRAIN'] + ['FDRR$^'+str(k)+'$' for k in K_parameter]
else:
    labels = ['SAA','Feature Driven', 'Feature Driven-L1', 'BASE-retrain','CVAR'] + ['FDRRref$^'+str(k)+'$' for k in K_parameter]+['FDRRaff$^'+str(k)+'$' for k in K_parameter]
...
for iter_ in range(iterations):
    for m_feat in num_del_group:
        ...
        for perc in percentage_obs:
            ...
            #CVAR 
            CVAR_pred  = cvar.predict(imp_X)
            CVAR_pred  = projection(CVAR_pred)
            ...
            if config['fixed_costs']:
                ...
                CVAR_mae = eval_dual_predictions(CVAR_pred,ytest.values, pen_up, pen_down)
            else:
...
models_to_plot = ['SAA', 'Feature Driven', 'Feature Driven-L1','BASE-retrain','CVAR']+  ['FDRR_affine'+str(k) for k in K_parameter]
...
marker = ['o', '2', '^','h', 'd', '1', '+', 's', 'v', '*', '^', 'p','+', '1', '2', '3','4']
...
models_to_plot = ['SAA', 'Feature Driven', 'Feature Driven-L1','BASE-retrain','CVAR'] +  ['FDRR_affine'+str(k) for k in K_parameter]
labels = ['SAA', 'Feature Driven', 'Feature Driven-L1','BASE-retrain','CVAR'] + ['FDRR_reform$('+str(k)+')$' for k in K_parameter] + ['FDRR_affine$('+str(k)+')$' for k in K_parameter]
fig,ax = plt.subplots(constrained_layout = True)
...
#plt.ylim(0,4)
if config['save']: 
    plt.savefig(f'{cd}\\{case_folder}\\figures\\DA-CVAR-accuracy-missing.png')
plt.show()
...
plot_models = ['SAA', 'Feature Driven', 'Feature Driven-L1','BASE-retrain','CVAR', 'FDRR-R', 'FDRR-AAR']    
plot_labels =['SAA', 'Feature Driven', 'Feature Driven-L1','BASE-retrain','CVAR', 'FDRR-R', 'FDRR-AAR']
...
if config['save']: 
    plt.savefig(f'{cd}\\{case_folder}\\figures\\DA-accuracy-missing.png')
plt.show()
...
color_list = ['tab:blue', 'tab:cyan','tab:green',
              'black', 'tab:brown', 'tab:purple','tab:red', 'tab:orange', 'tab:olive']
std_bar1 = temp_df.groupby(['percentage'])['SAA','Feature Driven', 'Feature Driven-L1','CVAR','BASE-retrain'].std()
std_bar2 = temp_df.groupby(['percentage'])['FDRR-R', 'FDRR-AAR'].std()
std_bar = pd.concat([std_bar1,std_bar2],ignore_index=False,axis = True)
fig,ax = plt.subplots(constrained_layout = True)
newdf1 = temp_df.groupby(['percentage'])['SAA','Feature Driven', 'Feature Driven-L1','CVAR','BASE-retrain'].mean()
...
if config['save']: 
    plt.savefig(f'{cd}\\{case_folder}\\figures\\DA-cvar-sensitivity')
plt.show()
